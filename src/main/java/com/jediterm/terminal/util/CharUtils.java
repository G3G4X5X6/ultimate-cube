...
public class CharUtils {
    ...
    public static int getTextLengthDoubleWidthAware(char[] buffer, int start, int length, boolean ambiguousIsDWC) {
        ...
        for (int i = start; i < start + length; i++) {
            result += (buffer[i] != com.jediterm.terminal.util.CharUtils.DWC) && isDoubleWidthCharacter(buffer[i], ambiguousIsDWC) && !((i + 1 < start + length) && (buffer[i + 1] == com.jediterm.terminal.util.CharUtils.DWC)) ? 2 : 1;
        }
        return result;
    }
